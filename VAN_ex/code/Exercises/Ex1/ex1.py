...
def read_images(idx):
    ...
    :param data_path string of the path for the images
...
def present_kp_locations(left_image, kp1, right_image, kp2):
    ...
    output_image2 = cv2.drawKeypoints(right_image, kp2, 0, (0, 0, 255),
                                      cv2.DRAW_MATCHES_FLAGS_NOT_DRAW_SINGLE_POINTS)
    cv2.imshow("1.1 - left image key-points", output_image1)
    cv2.imshow("1.1 - right image key-points", output_image2)
    cv2.waitKey(0)
